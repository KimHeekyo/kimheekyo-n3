...
function renderWords(){const q=document.querySelector('#wordSearch').value.trim().toLowerCase();const type=document.querySelector('#categoryFilter').value;const list=STUDY_WORDS.filter(w=>(type==='all'||w.type===type)&&[w.word,w.kana,w.meaning].some(x=>x.toLowerCase().includes(q)));const grid=document.querySelector('#wordGrid');grid.innerHTML=list.map(cardHtml).join('');document.querySelector('#emptyWords').classList.toggle('hidden',list.length>0);bindCards(grid)}
