...
function renderQuestion(){setQuizAwaiting(true);const w=STUDY_WORDS[quiz[quizIndex]];const jp=direction==='jp-ko';const reading=document.querySelector('#questionReading');const hint=document.querySelector('#readingHint');const canHint=jp&&w.kana!==w.word;document.querySelector('#quizProgress').textContent=quizMode==='random'?`${quizRoundOffset+quizIndex+1}번째 문제`:`${quizIndex+1} / ${quiz.length}`;document.querySelector('#quizScore').textContent=`정답 ${quizScore}`;document.querySelector('#progressBar').style.width=`${(quizIndex+1)/quiz.length*100}%`;document.querySelector('#questionLabel').textContent=jp?'다음 단어의 뜻은?':'다음 뜻에 맞는 일본어는?';document.querySelector('#questionText').textContent=jp?w.word:quizMeaning(w);reading.textContent=canHint?w.kana:'';reading.classList.add('hidden');hint.classList.toggle('hidden',!canHint);document.querySelector('#answerPanel').classList.add('hidden');const distractors=pickDistractors(w,jp);const opts=shuffle([w,...distractors]);const box=document.querySelector('#options');box.innerHTML=opts.map(x=>`<button class="option" data-correct="${x===w}">${jp?escapeHtml(quizMeaning(x)):wordWithReading(x)}</button>`).join('');box.querySelectorAll('button').forEach(b=>b.addEventListener('click',()=>answer(b,w)))}
function answer(button,w){setQuizAwaiting(false);document.querySelector('#readingHint').classList.add('hidden');document.querySelector('#questionReading').classList.remove('hidden');const correct=button.dataset.correct==='true';document.querySelectorAll('.option').forEach(b=>{b.disabled=true;if(b.dataset.correct==='true')b.classList.add('correct')});if(!correct)button.classList.add('wrong');if(correct){quizScore++;state.correct++;if(!state.mastered.includes(quiz[quizIndex]))state.mastered.push(quiz[quizIndex]);state.wrong=state.wrong.filter(i=>i!==quiz[quizIndex])}else if(!state.wrong.includes(quiz[quizIndex]))state.wrong.push(quiz[quizIndex]);state.answered++;save();document.querySelector('#answerResult').textContent=correct?'정답이에요! ✓':`아쉬워요. 정답은 ‘${direction==='jp-ko'?quizMeaning(w):w.word}’`;document.querySelector('#answerResult').style.color=correct?'#347050':'#bd5545';document.querySelector('#answerReading').textContent=w.kana!==w.word?`${w.word}（${w.kana}）`:w.word;document.querySelector('#nextQuestion').textContent=quizIndex===quiz.length-1&&quizMode==='daily'?'결과 보기 →':'다음 문제 →';document.querySelector('#answerPanel').classList.remove('hidden')}
function nextQuestion(){quizIndex++;if(quizIndex<quiz.length){renderQuestion();return}if(quizMode==='random'){quizRoundOffset+=quiz.length;quiz=shuffle(STUDY_WORDS.map((_,i)=>i));quizIndex=0;renderQuestion();return}document.querySelector('#quizGame').classList.add('hidden');document.querySelector('#quizResult').classList.remove('hidden');document.querySelector('#resultCopy').textContent=`10문제 중 ${quizScore}문제를 맞혔어요. ${quizScore>=8?'훌륭해요! 이 흐름을 이어가세요.':quizScore>=5?'좋아요! 헷갈린 단어를 한 번 더 볼까요?':'오답노트로 차근차근 익혀봐요.'}`}
