...
function showView(id){setQuizAwaiting(false);const home=id==='home';document.documentElement.classList.toggle('home-active',home);document.body.classList.toggle('home-active',home);document.querySelectorAll('.view').forEach(v=>v.classList.toggle('active',v.id===id));document.querySelectorAll('.nav-button').forEach(b=>b.classList.toggle('active',b.dataset.view===id));if(id==='words')renderWords();if(id==='review')renderReview();scrollTo({top:0,behavior:'smooth'})}
...
document.querySelector('#startQuiz').addEventListener('click',startQuiz);document.querySelector('#retryQuiz').addEventListener('click',startQuiz);document.querySelector('#nextQuestion').addEventListener('click',nextQuestion);document.querySelector('#quitQuiz').addEventListener('click',()=>{setQuizAwaiting(false);document.querySelector('#quizGame').classList.add('hidden');document.querySelector('#quizIntro').classList.remove('hidden')});
