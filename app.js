...
function showRecordedAnswer(record,w){setQuizAwaiting(false);document.querySelector('#readingHint').classList.add('hidden');document.querySelector('#questionReading').classList.remove('hidden');document.querySelectorAll('.option').forEach(b=>{b.disabled=true;if(b.dataset.correct==='true')b.classList.add('correct');if(Number(b.dataset.wordIndex)===record.selected&&!record.correct)b.classList.add('wrong')});document.querySelector('#answerResult').textContent=record.correct?'정답이에요! ✓':`아쉬워요. 정답은 ‘${direction==='jp-ko'?quizMeaning(w):w.word}’`;document.querySelector('#answerResult').style.color=record.correct?'#347050':'#bd5545';document.querySelector('#answerReading').textContent=w.kana!==w.word?`${w.word}（${w.kana}）`:w.word;document.querySelector('#nextQuestion').textContent=quizIndex===quiz.length-1&&quizMode==='daily'?'결과 보기 →':'다음 문제 →';document.querySelector('#answerPanel').classList.remove('hidden')}
