// Legacy correction tables below are retained only for migration history.
// The first-release study data is generated from data/manual-review.json.
const WORDS=[];
const LEGACY_WORDS_REVIEWED = [
  {word:'相変わらず',kana:'あいかわらず',meaning:'변함없이, 여전히',type:'부사',example:'彼は相変わらず元気です。',translation:'그는 여전히 건강합니다.'},
  {word:'明らか',kana:'あきらか',meaning:'명백하다, 분명하다',type:'형용사',example:'それは明らかな間違いです。',translation:'그것은 명백한 실수입니다.'},
  ...
  {word:'活発',kana:'かっぱつ',meaning:'활발하다',type:'형용사',example:'彼女は活発で明るい人です。',translation:'그녀는 활발하고 밝은 사람입니다.'},
  ...
  {word:'具体的',kana:'ぐたいてき',meaning:'구체적이다',type:'형용사',example:'具体的な例を説明してください。',translation:'구체적인 예를 설명해 주세요.'},
  ...
  {word:'積極的',kana:'せっきょくてき',meaning:'적극적이다',type:'형용사',example:'会議で積極的に意見を言いました。',translation:'회의에서 적극적으로 의견을 말했습니다.'},
...
// Only entries whose Korean meaning, reading, part of speech, example, and
// translation have all been reviewed are exposed to study modes.  The large
// imported bundle remains in data/words.js as a review queue, not as an answer
// key.  Re-apply the original hand-reviewed records before building the set.
const HAND_REVIEWED_WORDS = new Map(LEGACY_WORDS_REVIEWED.map(word=>[word.word,word]));
WORDS.forEach(word=>{const reviewed=HAND_REVIEWED_WORDS.get(word.word);if(reviewed)Object.assign(word,reviewed)});

const VOCABULARY_CORRECTIONS={
  'うがい':{meaning:'가글, 입 헹구기',type:'명사',translation:'가글했어?'},
  '夜':{kana:'よる',meaning:'밤, 저녁',type:'명사',example:'夜は静かです。',translation:'밤은 조용합니다.'},
  '今日':{kana:'きょう',meaning:'오늘',type:'명사·부사',example:'今日も頑張りましょう。',translation:'오늘도 힘냅시다.'},
  'すてき':{meaning:'멋지다, 근사하다',type:'형용사',example:'すてきな服ですね。',translation:'멋진 옷이네요.'},
...
const REVIEWED_WORD_KEYS=new Set([
  ...HAND_REVIEWED_WORDS.keys(),
  ...Object.keys(VOCABULARY_CORRECTIONS).map(word=>word==='いち'?'市':word),
  ...Object.keys(KATAKANA_CORRECTIONS)
]);
...
let state=loadState(); let quiz=[]; let quizIndex=0; let quizScore=0; let direction='jp-ko'; let quizMode='daily'; let quizRoundOffset=0; let quizAnswers=[];
function loadState(){const empty={answered:0,correct:0,mastered:[],wrong:[],date:todayKey(),datasetVersion:DATASET_VERSION};try{const saved={...empty,...JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}')};if(saved.datasetVersion!==DATASET_VERSION){saved.mastered=[];saved.wrong=[];saved.datasetVersion=DATASET_VERSION}return saved}catch{return empty}}
...
function pickDistractors(w,jp){const correctMeaning=quizMeaning(w);const seen=new Set([jp?correctMeaning:w.word]);return shuffle(STUDY_WORDS).filter(x=>{if(x===w||(!jp&&quizMeaning(x)===correctMeaning))return false;const displayed=jp?quizMeaning(x):x.word;if(seen.has(displayed))return false;seen.add(displayed);return true}).slice(0,3)}
...
function setQuizMode(mode){quizMode=mode;const endless=mode==='random';document.querySelector('#quizIntroLabel').textContent=endless?'ENDLESS QUIZ':'10 QUESTIONS';document.querySelector('#quizIntroTitle').textContent=endless?'무제한 랜덤 퀴즈':'오늘의 퀴즈';document.querySelector('#quizIntroCopy').innerHTML=endless?'전체 단어에서 문제가 계속 출제됩니다.<br>그만하고 싶을 때 나가기를 누르세요.':'힌트로 읽기를 확인할 수 있어요.<br>틀린 단어는 오답노트에 자동으로 담겨요.'}
function updateStats(){if(state.date!==todayKey()){state.answered=0;state.correct=0;state.date=todayKey();save();return}document.querySelector('#answeredStat').textContent=state.answered;document.querySelector('#accuracyStat').textContent=state.answered?Math.round(state.correct/state.answered*100)+'%':'—';document.querySelector('#accuracyCaption').textContent=state.answered?`${state.correct}개 정답`:'첫 문제를 풀어보세요';document.querySelector('#masteredStat').textContent=state.mastered.length;document.querySelector('#reviewCount').textContent=state.wrong.length;document.querySelector('#wordTotal').textContent=`전체 ${WORD_LIST_WORDS.length}단어`}
function cardHtml(w){const canHint=w.kana&&w.kana!==w.word;return `<article class="word-card"><span class="tag">${escapeHtml(w.type)}</span><h3>${escapeHtml(w.word)}</h3><div class="card-reading-slot">${canHint?`<button class="card-reading-hint" type="button">힌트보기</button><span class="kana hidden">${escapeHtml(w.kana)}</span>`:`<span class="kana">${escapeHtml(w.kana)}</span>`}</div><p class="meaning">${escapeHtml(w.meaning)}</p></article>`}
function bindCards(root){root.querySelectorAll('.card-reading-hint').forEach(button=>button.addEventListener('click',()=>{button.classList.add('hidden');button.nextElementSibling.classList.remove('hidden')}))}
function renderWords(){const q=document.querySelector('#wordSearch').value.trim().toLowerCase();const type=document.querySelector('#categoryFilter').value;const list=WORD_LIST_WORDS.filter(w=>(type==='all'||w.type===type)&&[w.word,w.kana,w.meaning].some(x=>x.toLowerCase().includes(q)));const grid=document.querySelector('#wordGrid');grid.innerHTML=list.map(cardHtml).join('');document.querySelector('#emptyWords').classList.toggle('hidden',list.length>0);bindCards(grid)}
...
function renderReview(){const list=state.wrong.map(i=>STUDY_WORDS[i]).filter(Boolean);const grid=document.querySelector('#reviewGrid');grid.innerHTML=list.map(cardHtml).join('');document.querySelector('#emptyReview').classList.toggle('hidden',list.length>0);bindCards(grid)}
function startQuiz(){setQuizIntroLayout(false);direction=document.querySelector('input[name="direction"]:checked').value;quiz=shuffle(STUDY_WORDS.map((_,i)=>i));if(quizMode==='daily')quiz=quiz.slice(0,10);quizIndex=0;quizScore=0;quizRoundOffset=0;quizAnswers=[];document.querySelector('#quizIntro').classList.add('hidden');document.querySelector('#quizResult').classList.add('hidden');document.querySelector('#quizGame').classList.remove('hidden');document.querySelector('#progressTrack').classList.toggle('hidden',quizMode==='random');renderQuestion()}
function renderQuestion(){const w=STUDY_WORDS[quiz[quizIndex]];const jp=direction==='jp-ko';const reading=document.querySelector('#questionReading');const hint=document.querySelector('#readingHint');const canHint=jp&&w.kana!==w.word;const previous=document.querySelector('#previousQuestion');previous.classList.toggle('hidden',quizIndex===0);document.querySelector('#quizProgress').textContent=quizMode==='random'?`${quizRoundOffset+quizIndex+1}번째 문제`:`${quizIndex+1} / ${quiz.length}`;document.querySelector('#quizScore').textContent=`정답 ${quizScore}`;document.querySelector('#progressBar').style.width=`${(quizIndex+1)/quiz.length*100}%`;document.querySelector('#questionLabel').textContent=jp?'다음 단어의 뜻은?':'다음 뜻에 맞는 일본어는?';document.querySelector('#questionText').textContent=jp?w.word:quizMeaning(w);reading.textContent=canHint?w.kana:'';reading.classList.add('hidden');hint.classList.toggle('hidden',!canHint);document.querySelector('#answerPanel').classList.add('hidden');let record=quizAnswers[quizIndex];if(!record){const distractors=pickDistractors(w,jp);record={options:shuffle([w,...distractors]).map(x=>STUDY_WORDS.indexOf(x)),selected:null,correct:null};quizAnswers[quizIndex]=record}const box=document.querySelector('#options');box.innerHTML=record.options.map(index=>{const x=STUDY_WORDS[index];return `<button class="option" data-word-index="${index}" data-correct="${index===quiz[quizIndex]}">${jp?escapeHtml(quizMeaning(x)):wordWithReading(x)}</button>`}).join('');box.querySelectorAll('button').forEach(b=>b.addEventListener('click',()=>answer(b,w)));if(record.selected!==null)showRecordedAnswer(record,w);else setQuizAwaiting(true)}
function showRecordedAnswer(record,w){setQuizAwaiting(false);document.querySelector('#readingHint').classList.add('hidden');document.querySelector('#questionReading').classList.remove('hidden');document.querySelectorAll('.option').forEach(b=>{b.disabled=true;if(b.dataset.correct==='true')b.classList.add('correct');if(Number(b.dataset.wordIndex)===record.selected&&!record.correct)b.classList.add('wrong')});document.querySelector('#answerResult').textContent=record.correct?'정답이에요! ✓':`아쉬워요. 정답은 ‘${direction==='jp-ko'?quizMeaning(w):w.word}’`;document.querySelector('#answerResult').style.color=record.correct?'#347050':'#bd5545';document.querySelector('#answerReading').textContent=w.kana!==w.word?`${w.word}（${w.kana}）`:w.word;document.querySelector('#exampleJp').textContent=w.example;document.querySelector('#exampleKo').textContent=w.translation;document.querySelector('#nextQuestion').textContent=quizIndex===quiz.length-1&&quizMode==='daily'?'결과 보기 →':'다음 문제 →';document.querySelector('#answerPanel').classList.remove('hidden')}
function answer(button,w){const record=quizAnswers[quizIndex];if(record.selected!==null)return;record.selected=Number(button.dataset.wordIndex);record.correct=button.dataset.correct==='true';if(record.correct){quizScore++;state.correct++;if(!state.mastered.includes(quiz[quizIndex]))state.mastered.push(quiz[quizIndex]);state.wrong=state.wrong.filter(i=>i!==quiz[quizIndex])}else if(!state.wrong.includes(quiz[quizIndex]))state.wrong.push(quiz[quizIndex]);state.answered++;save();showRecordedAnswer(record,w)}
function nextQuestion(){quizIndex++;if(quizIndex<quiz.length){renderQuestion();return}if(quizMode==='random'){quizRoundOffset+=quiz.length;quiz=shuffle(STUDY_WORDS.map((_,i)=>i));quizIndex=0;quizAnswers=[];renderQuestion();return}document.querySelector('#quizGame').classList.add('hidden');document.querySelector('#quizResult').classList.remove('hidden');document.querySelector('#resultCopy').textContent=`10문제 중 ${quizScore}문제를 맞혔어요. ${quizScore>=8?'훌륭해요! 이 흐름을 이어가세요.':quizScore>=5?'좋아요! 헷갈린 단어를 한 번 더 볼까요?':'오답노트로 차근차근 익혀봐요.'}`}
document.querySelector('#startQuiz').addEventListener('click',startQuiz);document.querySelector('#retryQuiz').addEventListener('click',startQuiz);document.querySelector('#nextQuestion').addEventListener('click',nextQuestion);document.querySelector('#previousQuestion').addEventListener('click',()=>{if(quizIndex>0){quizIndex--;renderQuestion()}});document.querySelector('#readingHint').addEventListener('click',()=>{document.querySelector('#readingHint').classList.add('hidden');document.querySelector('#questionReading').classList.remove('hidden')});document.querySelector('#quitQuiz').addEventListener('click',()=>{setQuizAwaiting(false);setQuizIntroLayout(true);document.querySelector('#quizGame').classList.add('hidden');document.querySelector('#quizIntro').classList.remove('hidden')});
