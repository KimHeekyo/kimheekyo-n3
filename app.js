...
const STUDY_WORDS=WORDS.filter(word=>REVIEWED_WORD_KEYS.has(word.word));
// The dictionary is also the review catalogue, so every imported N3 entry must
// remain discoverable here. Quiz and review modes deliberately use only the
// independently checked answer set above.
const WORD_LIST_WORDS=WORDS;
...
function setQuizMode(mode){quizMode=mode;const endless=mode==='random';document.querySelector('#quizIntroLabel').textContent=endless?'ENDLESS QUIZ':'10 QUESTIONS';document.querySelector('#quizIntroTitle').textContent=endless?'무제한 랜덤 퀴즈':'오늘의 퀴즈';document.querySelector('#quizIntroCopy').innerHTML=endless?'검수 완료 단어에서 문제가 계속 출제됩니다.<br>그만하고 싶을 때 나가기를 누르세요.':'뜻을 맞히면 예문이 공개됩니다.<br>틀린 단어는 오답노트에 자동으로 담겨요.'}
function updateStats(){if(state.date!==todayKey()){state.answered=0;state.correct=0;state.date=todayKey();save();return}document.querySelector('#answeredStat').textContent=state.answered;document.querySelector('#accuracyStat').textContent=state.answered?Math.round(state.correct/state.answered*100)+'%':'—';document.querySelector('#accuracyCaption').textContent=state.answered?`${state.correct}개 정답`:'첫 문제를 풀어보세요';document.querySelector('#masteredStat').textContent=state.mastered.length;document.querySelector('#reviewCount').textContent=state.wrong.length;document.querySelector('#wordTotal').textContent=`전체 ${WORD_LIST_WORDS.length}단어 · 퀴즈 검수 ${STUDY_WORDS.length}단어`}
